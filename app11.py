...
def auto_def2():
    ...
    if uploaded_file is not None:
        ...
        st.subheader('용해 온도 Line Chart')
        st.line_chart(MELT_TEMP[:100])
        st.title(' ')
        st.title(' ')
   
        st.subheader('용해 교반속도 Line Chart')
        st.line_chart(MOTORSPEED[:100])
        st.title(' ')
        st.title(' ')
        ...
        st.title(' ')
        st.subheader('용해탱크의 이상탐지 결과는 다음과 같습니다.')
...
if selected_box == '데이터 입력 및 결과':
    auto_def1()
    st.sidebar.write("모바일에서는 상단의 X를 눌러 원래화면으로 가세요")
    

if selected_box == '파일 불러오기 및 결과':
    auto_def2()
    st.sidebar.write("모바일에서는 상단의 X를 눌러 원래화면으로 가세요")
